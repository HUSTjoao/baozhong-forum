...
import { alumniMessagesApi, type AlumniMessage } from '@/lib/api-client'
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    if (session?.user?.id && messages.length > 0) {
      ...
      messages.forEach(msg => {
        if (msg.likesRel?.some(like => like.userId === session.user.id)) {
          likedSet.add(msg.id)
        }
      })
  ...
  const loadMessages = async () => {
    try {
      const data = await alumniMessagesApi.list()
      setMessages(data)
    } catch (error: any) {
      console.error('加载寄语失败:', error)
      showAlert('加载失败', error.message || '加载寄语列表失败')
    }
  }
  ...
  const handleSaveMessage = async () => {
    ...
    try {
      await alumniMessagesApi.create(messageText.trim())
      await loadMessages()
      setIsWriting(false)
      setMessageText('')
      showAlert('寄语发布成功', '你的寄语已经展示在下方列表中啦～')
    } catch (error: any) {
      showAlert('保存失败', error.message || '请稍后重试')
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await alumniMessagesApi.delete(pendingDeleteId)
      await loadMessages()
      // 更新点赞状态
      ...
      showAlert('删除失败', error.message || '删除失败，请稍后重试')
    }
  ...
  const handleLike = async (messageId: string) => {
    ...
    try {
      const updated = await alumniMessagesApi.toggleLike(messageId)
      
      // 更新本地状态
      const newLikedSet = new Set(likedMessages)
      if (updated.likesRel?.some(like => like.userId === session.user.id)) {
      ...
      setMessages(prev => prev.map(msg =>
        msg.id === messageId
          ? { ...msg, likes: updated.likes, likesRel: updated.likesRel }
          : msg
      ))
      ...
      showAlert('点赞失败', error.message || '请稍后重试')
    }
  }
