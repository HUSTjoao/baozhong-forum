...
function getUserAcademicLabel(user?: User): string | null {
  if (!user) return null
...
export default function QuestionDetailPage({
  ...
  useEffect(() => {
    const found = getQuestionById(params.id)
    if (!found) {
      router.push('/forum')
      return
    }
  ...
  const handleQuestionLike = () => {
    ...
    const updated = toggleQuestionLike(question.id, session.user.id)
    if (updated) {
      setQuestion(updated)
    }
  }
  ...
  const handleReplyLike = (replyId: string) => {
    ...
    const updated = toggleReplyLike(question.id, replyId, session.user.id)
    if (updated) {
      // 重新加载问题数据以确保嵌套回复的更新正确反映
      const reloaded = getQuestionById(question.id)
      if (reloaded) {
        setQuestion(reloaded)
      }
    }
  }
  ...
  const handleSubmitReply = async (e: React.FormEvent) => {
    ...
    const newReply = addReply(question.id, {
      replier: isReplyAnonymous ? '匿名用户' : (session.user.nickname || session.user.name || session.user.email || '匿名用户'),
      replierId: isReplyAnonymous ? undefined : session.user.id,
      date: new Date().toISOString(),
      content: replyContent.trim(),
      isAlumni: session.user.role === 'alumni',
      isAnonymous: isReplyAnonymous,
    })

    if (newReply) {
      // 重新加载问题数据
      const updated = getQuestionById(question.id)
      if (updated) {
        setQuestion(updated)
      }
      setReplyContent('')
  ...
  const handleSubmitReplyToReply = async (parentReplyId: string, parentReplier: string) => {
    ...
    const newReply = addReply(question.id, {
      replier: replyingAnonymous ? '匿名用户' : (session.user.nickname || session.user.name || session.user.email || '匿名用户'),
      replierId: replyingAnonymous ? undefined : session.user.id,
      date: new Date().toISOString(),
  ...
  // 返回链接逻辑：
  // - 如果有 majorId：优先回到对应专业论坛
  // - 否则如果有 universityId：回到该大学的讨论区
  // - 否则：回到总问答论坛
  let backHref = '/forum'
  ...
  const handleConfirmDelete = () => {
    ...
    if (deleteConfirm.type === 'question') {
      ...
      const deleted = deleteReply(
        question.id,
        deleteConfirm.replyId,
        isAdmin ? 'admin' : session.user.id,
        { isAdmin }
      )
      if (deleted) {
  ...
  const handleReportSubmit = (e: React.FormEvent) => {
    ...
    const report = addReport({
      ...
      reporterName: session.user.nickname || session.user.name || session.user.email || '匿名用户',
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {question.isAnonymous || !question.askerId ? (
                    <span className="font-medium">
                      {question.isAnonymous ? '匿名用户' : question.asker}
                    </span>
                  ) : (
                    <Link
                      href={`/users/${question.askerId}`}
        ...
        <div className="rounded-2xl border border-slate-100/80 bg-slate-50/95 p-7 md:p-8 shadow-[0_18px_40px_rgba(15,23,42,0.12)] backdrop-blur-xl">
          ...
          {question.replies > 0 ? (
            <div className="space-y-5">
              {question.replyList?.filter((r) => !r.parentReplyId).map((reply) => (
                <ReplyItem
                  key={reply.id}
                  reply={reply}
                  questionId={question.id}
            ...
            {reply.isAnonymous || !reply.replierId ? (
              ...
              >
                ...
                <img
                  ...
                  alt={userDetail.nickname || userDetail.name || reply.replier}
            ...
            <div className="flex items-center space-x-2 flex-wrap">
              {reply.isAnonymous || !reply.replierId ? (
                <span className="font-semibold text-gray-800">
                  {reply.isAnonymous ? '匿名用户' : reply.replier}
                </span>
              ) : (
                <Link
                  href={`/users/${reply.replierId}`}
              ...
              {!reply.isAnonymous && reply.isAlumni && (
          ...
          <div className="flex items-center space-x-2">
            ...
            {session && (
              <>
                ...
                <button
                  onClick={() =>
                    setReplyingTo(isReplying ? null : { replyId: reply.id, replier: reply.replier })
                  }
          ...
          <div className="mt-3 mb-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="space-x-2">
                ...
                <button
                  onClick={() => onReplyToReply(reply.id, reply.replier)}
